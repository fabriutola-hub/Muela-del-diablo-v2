  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 border-b-3 ${isScrolled || menuOpen
          ...
          <nav className="hidden md:flex items-center gap-4">
            {menuItems && menuItems.map((item) => (
              <button
                ...
                className={`text-sm font-bold font-mono uppercase hover:underline decoration-4 decoration-arcilla underline-offset-4 transition-all ${isScrolled ? 'text-negro-illimani' : 'text-white drop-shadow-[1px_1px_0px_black]'
                  }`}
              >
                {item.name}
              </button>
